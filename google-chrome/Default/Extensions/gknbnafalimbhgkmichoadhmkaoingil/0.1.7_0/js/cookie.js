DebugCookie = {
	...
	get: function(url, names, fn){
		chrome.cookies.getAll({domain: url.hostname}, function(cookies) {
			var _cookies = {length:0};
			for(var i=0; i<cookies.length; ++i){
				if($.inArray(cookies[i].name, names) > -1){
					_cookies[cookies[i].name] = cookies[i];
					_cookies.length++;
				};
				// If our list of cookies is equal to the list of provided names, we are done, break out the loop.
	...
	update: function(data, url){
		for(var n in data){
			if(data[n] != ''){
				...
				chrome.cookies.set({name: n, value: data[n].toString(), url: url.protocol + '//' + url.host});
			}else{
				// If the property is empty, remove the cookie
				chrome.cookies.remove({name: n, url: url.protocol + '//' + url.host});
			};
		};
	}
};